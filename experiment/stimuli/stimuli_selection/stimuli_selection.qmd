...
```{r}
q <- 0.75  # Quantile (adjust so to meet target N of stimuli)
...
```{r}
files <- str_split(dffinal$File, "/", simplify = TRUE)[,12]  # Change index to take last element
...
json <- selection |>
  mutate(Orientation = ifelse(Width > Height, "h", "v")) |>
  select(Item, Style, Orientation) |>
  jsonlite::toJSON()
