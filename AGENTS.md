...
│   ├── 7_correlates.qmd
│   ├── report.R            # table helpers + read_estimates(), shared by 3_ and 4_
...
| 7 | `analysis/7_correlates.qmd` | `models/individual/*.rds` (`./hpc individual`, `get_individual()` in `server/estimates.R`) | HTML report + `7_correlates.html.md`; participant-level indices (link-scale Baseline, Forgery − Original, AI − Original per dpar) |
