...
df <- read.csv("../data/data_memory_task.csv") |> 
  left_join(select(read.csv("../data/data_task.csv"), -Condition, -Reality, -Authenticity), by = c("Participant", "Item")) |>
...
The mean and median days between the two experiments were `r format_number(mean(dfsub$Delay))` and `r format_number(median(dfsub$Delay))`, respectively.
...
df2 <- dfsub |> 
  select(Participant, Gender, Age, Delay, Art_Expertise, 
