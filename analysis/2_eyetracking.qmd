...
```{r}
vel_threshold <- 5  # 5 stimulus-widths/sec ≈ upper bound of real saccade
...
df_vel |>
  ...
  ggplot(aes(x = pmin(velocity, 15), fill = Dropped)) +   # cap at 15 for readability
  ...
  labs(title = "Sample velocity",
       ...
       x = "Velocity (stim-widths / sec; capped at 15)", y = "Count", fill = NULL)
...
thresh_entropy     <- 0.95   
thresh_gini        <- 0.20   # below = suspiciously flat KDE
...
df_features <- df_features |>
  mutate(
    Status = case_when(
      ...
      gini_kde < thresh_gini |
        entropy5 > thresh_entropy ~
...
p_gini <- df_features |>
  ...
  geom_histogram(bins = 40, alpha = 0.8, position = "stack") +
  geom_vline(xintercept = thresh_gini,
             linetype = "dashed", color = "black", linewidth = 0.8) +
  annotate("text", x = thresh_gini, y = Inf,
           label = paste0("threshold = ", thresh_gini),
           vjust = 1.5, hjust = -0.1, size = 3) +
  scale_fill_manual(values = c("FALSE" = "#4CAF50", "TRUE" = "firebrick"),
                    labels = c("Kept", "Dropped")) +
  theme_minimal() +
...
df_num <- df_features |>
  select(-Participant, -Item, -entropy) |>
