...
outliers$manipulation <- filter(manipcheck, Confidence >= 4)$Participant
...
phase2congruence |> 
  ggplot(aes(x = Reality_Human, y = Authenticity_Human, color = log(Duration_TaskInstructions2))) +
  geom_point(shape = "+", size = 5) +
  ggrepel::geom_label_repel(data = filter(phase2congruence, Reality_Human > 0.8), 
...
hist(rez$reachdist)
clusters <- dbscan::extractDBSCAN(rez, eps_cl = quantile(rez$reachdist, 0.66))
