...
# Metaclusters: average of all their items. Every facet has 3 items (none
# missing), so this is the mean of the facet scores
df$MINT_Awareness <- rowMeans(df[c("MINT_ExAc", "MINT_RelA", "MINT_SexS")])
df$MINT_Deficit <- rowMeans(df[c("MINT_CaCo", "MINT_Urin", "MINT_Derm", "MINT_Sati", "MINT_Olfa")])
df$MINT_Visceroception <- rowMeans(df[c("MINT_Card", "MINT_Resp", "MINT_Gast")])
...
```{r}
# Items are coded 0 (no image) to 4 (as vivid as real seeing) in 0_preprocessing.R,
# so higher = more vivid without reversing (it was reversed until 2026-09-26)
...
TODO: check why we have missing values.
...
dfchecks <- dftask |> 
  ...
  mutate(cor_BeautyValence = ifelse(is.na(cor_BeautyValence), -1, cor_BeautyValence),
         ...
         cor_ValenceNorm = ifelse(is.na(cor_ValenceNorm), -1, cor_ValenceNorm)) |>  # NA when a participant gives a constant rating (zero variance)
...
estimate_relation(m, length=100) |> 
  ...
  # ggside::geom_ysidehistogram(data=filter(dfchecks, !is.na(A_TASK)), aes(y=A_TASK), alpha=0.3) +
  ggside::theme_ggside_void() 
```
...
exclude$anomaly <- exclude$anomaly[!exclude$anomaly %in% c(exclude$duration, exclude$checks)]
...
To transparently quantify the effect of participants exclusion, and validate our exclusion criteria, we provide an approximation of the main results with and without the excluded participants, which helps ensuring that the exclusion criteria do not bias the results.
...
quick_test <- function(what = "Beauty", family = "gaussian", exclude = exclude){
  ...
  # Models
  m_all <- glmmTMB::glmmTMB(f, data = dat, family = family)
  params <-  parameters::parameters(m_all, effects = "fixed")
  
  m_checks <- glmmTMB::glmmTMB(f, data = filter(dat, !Participant %in% exclude$checks), family = family)
  params2 <-  parameters::parameters(m_checks, effects = "fixed")
  
  m_dura <- glmmTMB::glmmTMB(f, data = filter(dat, !Participant %in% c(exclude$checks, exclude$duration)), family = family)
  params3 <-  parameters::parameters(m_dura, effects = "fixed")
  
  m_ano <- glmmTMB::glmmTMB(f, data = filter(dat, !Participant %in% c(exclude$checks, exclude$duration, exclude$anomaly)), family = family)
  params4 <-  parameters::parameters(m_ano, effects = "fixed")
  
  # Plot
  p <- estimate_means(m_all, by = "Condition") |> 
    mutate(Exclusion = "Whole sample") |> 
    rbind(estimate_means(m_checks, by = "Condition") |> 
            mutate(Exclusion = "Failed Attention Checks"),
          estimate_means(m_dura, by = "Condition") |> 
            mutate(Exclusion = "Failed Attention Checks + Duration"),
          estimate_means(m_ano, by = "Condition") |> 
            mutate(Exclusion = "Failed Attention Checks + Duration + Anomaly score")) 
  ...
  # table
  rez <- data.frame(Parameter = params$Parameter, z_all = params$z, z_checks = params2$z, z_duration = params3$z, z_anomaly = params4$z)
  ramp   <- colorRamp(c("white", "#4CAF50"))
  to_hex <- \(x) rgb(ramp(x), maxColorValue = 255)
  
  z_cols <- grep("^z_", names(rez), value = TRUE)
  
  # Per-row colour matrix for rows 2..n
  color_mat <- rez |>
    slice(-1) |>
    ...
    apply(1, \(row) {
      ...
      s   <- if (diff(rng) == 0) rep(1, length(av)) else (av - rng[1]) / diff(rng)
      to_hex(s)
    }) |>
    t() 
  
  # All (row, col) index pairs
  idx <- expand.grid(i = seq_len(nrow(color_mat)), j = seq_along(z_cols))
  
  # Base table: row 1 uniformly grey
  base <- rez |>
    gt::gt() |>
    gt::fmt_number(starts_with("z_"), decimals = 1) |>
    gt::tab_style(gt::cell_fill("lightgrey"),gt:: cells_body(all_of(z_cols), rows = 1))
  
  # Fold one tab_style per cell
  tab <- Reduce(
    \(tbl, k) gt::tab_style(tbl,
      gt::cell_fill(color_mat[idx$i[k], idx$j[k]]),
      gt::cells_body(z_cols[idx$j[k]], rows = idx$i[k] + 1L)),
    seq_len(nrow(idx)),
    init = base
  )


  list(params = tab, plot = p)
}
```
...
```{r}
rez <- quick_test(what = "Beauty", family = "gaussian", exclude = exclude)
rez$params
rez$plot  

rez <- quick_test(what = "Beauty", family = "ordbeta", exclude = exclude)
rez$params
rez$plot  
```
...
```{r}
rez <- quick_test(what = "Valence", family = "gaussian", exclude = exclude)
rez$params
rez$plot  

rez <- quick_test(what = "Valence", family = "ordbeta", exclude = exclude)
rez$params
rez$plot  
```
...
```{r}
rez <- quick_test(what = "Meaning", family = "gaussian", exclude = exclude)
rez$params
rez$plot  

rez <- quick_test(what = "Meaning", family = "ordbeta", exclude = exclude)
rez$params
rez$plot  
```
...
```{r}
rez <- quick_test(what = "Worth", family = "poisson", exclude = exclude)
rez$params
rez$plot
```
...
```{r}
rez <- quick_test(what = "Reality", family = "gaussian", exclude = exclude)
rez$params
rez$plot

rez <- quick_test(what = "Reality", family = "ordbeta", exclude = exclude)
rez$params
rez$plot
```
...
```{r}
rez <- quick_test(what = "Authenticity", family = "gaussian", exclude = exclude)
rez$params
rez$plot

rez <- quick_test(what = "Authenticity", family = "ordbeta", exclude = exclude)
rez$params
rez$plot
```
...
df <- filter(df, !Participant %in% c(exclude$checks))
dftask <- filter(dftask, !Participant %in% c(exclude$checks))
```
...
<!--     cor_BeautyMeaning = cor(Beauty, Meaning), -->
<!--     cor_MeaningWorth = cor(Meaning, Worth), -->
<!--     cor_MeaningValence = cor(Meaning, Valence), -->
<!--     .by = "Participant" -->
<!--   ) |>  -->
...
dftask <- dftask |> 
  ...
  left_join(select(read.csv("../data/rawdata_memory_task.csv"), Participant, Item, SelfRelevance, Beauty2, PerceivedArtificiality), by = c("Participant", "Item")) |> 
  mutate(Emotion = paste0(ifelse(Norms_Valence < median(Norms_Valence), "Negative", "Positive"), ifelse(Norms_Arousal < median(Norms_Arousal), " - Low intensity", " - High intensity")),
         ...
         PerceivedArtificiality = datawizard::rescale(PerceivedArtificiality, to = c(0, 1), range = c(-1, 1))
         ) |>
...
dfmem <- dfmem |> 
  mutate(Beauty2 = datawizard::rescale(Beauty2, to = c(0, 1), range = c(-3, 3)),
         SelfRelevance = datawizard::rescale(SelfRelevance, to = c(0, 1), range = c(0, 6)),
         PerceivedArtificiality = datawizard::rescale(PerceivedArtificiality, to = c(0, 1), range = c(-1, 1))) |> 
  filter(Participant %in% df$Participant) |> 
  left_join(select(dftask, -SelfRelevance, -Beauty2, -PerceivedArtificiality), by = c("Participant", "Item")) |>
