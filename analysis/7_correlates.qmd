...
#| fig-cap: "Correlations between the reliable memory indices (columns) and participant characteristics (220 participants), ordered within each group by the similarity of their correlation profiles (Interoception: each Mint metacluster followed by its facets, in italics). Bold: p < .05 (uncorrected); *: p < .05 after FDR correction across the whole table."
#| fig-width: 11
#| fig-height: 13.2
...
```{r}
#| label: fig-interindividual
#| fig-cap: "Structure and correlates of the participant-level indices. Top: correlations between the indices entering the EGA (lower half), their reliability (diagonal) and their hierarchical community structure (upper half). Bottom: correlations between participant characteristics (rows) and the lower-order community scores and the follow-up label recall tendencies (columns: the tendency to recall a recognised work as having had each label, whatever its actual label)."
#| fig-alt: "Top: triangular correlation matrix of the participant-level indices with a tree of their lower- and higher-order communities drawn above the diagonal. Bottom: heatmap of correlations between participant characteristics and the community scores and label recall tendencies."
#| fig-width: 20
#| fig-height: 30
