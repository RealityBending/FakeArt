...
#   FA_WARMUP          warmup iterations per chain          (default 5000)
#   FA_SAMPLES         post-warmup iterations per chain     (default 800)
#   FA_THIN            keep every n-th draw                 (default 2)
...
#
# The defaults are the settings the July 2026 production fits used
# (8 shards x 2 chains x 800 / 2 = 6,400 draws per model); see AGENT.md.
...
warmup <- as.integer(Sys.getenv("FA_WARMUP", unset = "5000"))
samples <- as.integer(Sys.getenv("FA_SAMPLES", unset = "800"))
thin <- as.integer(Sys.getenv("FA_THIN", unset = "2"))
iter <- warmup + samples
seed <- 1234 + task_id # distinct per shard, reproducible per shard

cat(sprintf(
  "config: model=%s outcome=%s shard=%d warmup=%d samples=%d thin=%d chains=%d threads/chain=%g refit=%s\n",
  spec$name, spec$outcome, task_id, warmup, samples, thin,
...
# FA_DATA=<dir> reads the two CSVs from a local directory instead (smoke tests
# on a laptop).
