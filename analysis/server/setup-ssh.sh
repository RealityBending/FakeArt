...
# Idempotent: safe to re-run. Shared across projects: if another repo
# (e.g. IllusionGameComputational) already set this machine up, it reports
# that and does nothing.
# ---------------------------------------------------------------------------
set -euo pipefail

HPC_USER="${FA_HPC_USER:-dmm56}"
...
---------------------------------------------------------------------------
Next: install this machine's public key on Artemis (skip if this machine's
key is already in ~/.ssh/authorized_keys there -- './hpc check' will tell).
